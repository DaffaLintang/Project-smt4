...
#     # One-hot encoding untuk Gender
#     categorical_encoded = encoder.transform(input_data[['Gender']]).toarray()
#     input_final = pd.concat([
#         pd.DataFrame(categorical_encoded, columns=encoder.get_feature_names_out()),
#         input_data[['Age', 'Height', 'Weight', 'BMI']].reset_index(drop=True)
#     ], axis=1)
...
#     height = 1.65
#     weight = 70
#     bmi = weight / (height ** 2)  # Pastikan BMI dihitung benar

#     result = predict_obesity(age, gender, height, weight, bmi)
